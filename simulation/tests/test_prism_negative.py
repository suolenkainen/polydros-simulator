...
def test_agent_prism_never_negative() -> None:
    ...
    for agent in result["agents"]:
        msg = f"Agent {agent['id']} has negative Prism: {agent['prism']}"
        assert agent["prism"] >= 0.0, msg
...
def test_agent_prism_starts_at_200() -> None:
    ...
    for agent in result["agents"]:
        msg = f"Agent {agent['id']} should start with 200.0 Prism"
        assert agent["prism"] == 200.0, f"{msg}, got {agent['prism']}"
...
def test_agent_cannot_buy_without_sufficient_prism() -> None:
    ...
    for agent in result["agents"]:
        msg = f"Agent {agent['id']} ended with negative Prism"
        assert agent["prism"] >= 0.0, f"{msg}: {agent['prism']}"
        ...
        # Starting with 200 Prism, they can make at most 3 purchases before
        # running out (200 - 60 - 60 - 60 = 20 Prism remaining)
        msg = f"Agent {agent['id']} made {len(purchases)} purchases, expected <=4"
        assert len(purchases) <= 4, msg
