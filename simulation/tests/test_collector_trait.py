...
The collector trait determines if agents continue buying boosters after 60 cards.
...
class TestCollectorTraitGeneration:
    ...
    def test_collector_trait_in_range(self):
        ...
        for agent in agents:
            ...
            ), f"Collector trait {trait_value} out of range"
...
class TestCollectorTraitBuying:
    ...
    def test_always_buy_until_60_cards(self):
        """Agents should always buy 5 packs per tick until 60 cards."""
        ...
        assert (
            collection_count >= 55
        ), f"Agent should have ~60 cards, got {collection_count}"
    ...
    def test_collector_trait_blocks_purchases_after_60_cards(self):
        """After 60 cards, purchases only occur if trait triggers."""
        ...
        # With varying collector trait, verify trait is in range
        # With a low collector trait (closer to 0.10), fewer purchases after 60
        # With a high collector trait (closer to 0.50), more purchases after 60
        assert 0.10 <= collector_trait <= 0.50

    def test_deterministic_collector_decisions(self):
        """Same seed produces same collector trait purchase decisions."""
        ...
        trait1 = agent1["traits"]["collector_trait"]
        trait2 = agent2["traits"]["collector_trait"]
        assert trait1 == trait2
