...
def test_tick_zero_initialization():
    ...
    for agent in result["agents"]:
        ...
        assert agent["prism"] == 200.0, f"Agent should start with 200 Prism, got {agent['prism']}"
        assert agent["collection_count"] == 0, f"Agent should have 0 cards at tick 0, got {agent['collection_count']}"
        assert agent["booster_count"] == 0, f"Agent should have 0 boosters at tick 0, got {agent['booster_count']}"
    
    # Check timeseries has only initial state
    assert len(result["timeseries"]) == 1, "Should have only tick 0 (initial state)"
