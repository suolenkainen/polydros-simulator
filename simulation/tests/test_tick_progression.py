...
def test_simulation_tick_progression() -> None:
    ...
    evt_type = "booster_purchase"
    agent_events_1 = [
        e for e in result1["events"] if e["event_type"] == evt_type
    ]
    assert len(agent_events_1) > 0, "Should have purchase events"


def test_simulation_incremental_runs_same_seed() -> None:
    """Test that running simulation incrementally produces same results."""
    ...
    _ = run_simulation(config_part1)  # Intermediate result not used
    ...
    for agent_id in full_agents:
        full_prism = full_agents[agent_id]["prism"]
        part2_prism = part2_agents[agent_id]["prism"]
        assert full_prism == part2_prism

        full_count = full_agents[agent_id]["collection_count"]
        part2_count = part2_agents[agent_id]["collection_count"]
        assert full_count == part2_count
...
def test_zero_ticks_simulation() -> None:
    ...
    evt_type = "booster_purchase"
    purchase_events = [
        e for e in result["events"] if e["event_type"] == evt_type
    ]
...
def test_agent_prism_after_tick_sequence() -> None:
    ...
    # With seed 42, collector_trait=0.21 (21%), random rolls likely don't trigger
    expected_msg = (
        "Expected 140.0 Prism after 3 ticks "
        "(collector trait didn't trigger), got"
    )
    assert (
        agent2["prism"] == 140.0
    ), f"{expected_msg} {agent2['prism']}"
    assert (
        agent2["collection_count"] >= 60
