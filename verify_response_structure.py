    ...
    if 'card_instances' in agent and agent['card_instances']:
        ...
        if 'price_history' in instance:
            ...
            for point in history[:3]:
                tick = point['tick']
                price = point['price']
                desirability = point['desirability']
                msg = f"  Tick {tick}: Price {price} Prisms, "
                msg += f"Desirability {desirability}"
                print(msg)

print("\n✓ Data structure verified - agents with card_instances and")
print("price_history present!")
